...
-- Target/Focus frames
UnitFrames.playerFrame = PlayerFrame
UnitFrames.targetFrame = TargetFrame
UnitFrames.focusFrame = FocusFrame
...
local function AddCombatIconTextureTo(frame)
	-- TODO: change size of icon to match PlayerFrame default combat indicator
	local texture = frame:CreateTexture(nil, "BACKGROUND")

	texture:SetTexture("Interface\\CHARACTERFRAME\\UI-StateIcon.blp")
	texture:SetTexCoord(.56, 1, 0, .47)
...
local function CreateCombatIconFrameOn(parentFrame)
...
local function ToggleCombatIndicator(self, frame, unit)
...
function UnitFrames:HandleCombatIndicator(enable)
	self.db.combatIndicator = enable

	self.targetCombatIconFrame = CreateCombatIconFrameOn(self.targetFrame)
	self.focusCombatIconFrame = CreateCombatIconFrameOn(self.focusFrame)
	
	-- Combat indicator is enabled
	if enable then
		EventHandler:SetScript("OnUpdate", function()
			ToggleCombatIndicator(
									self.targetFrame,
									self.targetCombatIconFrame,
									"target"
								)
			ToggleCombatIndicator(
									self.focusFrame,
									self.focusCombatIconFrame,
									"focus"
								)
		end)
	else
		-- Combat indicator is disabled
		EventHandler:SetScript("OnUpdate", nil)
		
		-- Hide combat indicator frames
		self.targetCombatIconFrame:Hide()
		self.focusCombatIconFrame:Hide()

		-- Show character level
		self.targetFrame.levelText:Show()
		self.focusFrame.levelText:Show()
	end
end
